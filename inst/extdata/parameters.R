...
hyperparms <- list(
  ...
  tbi_protn=list(shape1=20.7,shape2=77.9),            #psi:protn Andrews
  ...
  relapse=list(meanlog=-3.95,sdlog=0.27),         #omega: relapse Crampin NOTE x-ref
  ## --------------------------------------------------- detection
  ...
  progn_symp = list(meanlog=-0.693,sdlog=0.97),  #durn AS TB D from getLNparms(0.5,1) = Frascella half of TB SC + 1ydrn
  ## mHR = list(meanlog=0.131,sdlog=0.071),             #post-TB mortality HR
  treatment_inversedurn = 2
